# discharge_planning_groq.py
"""
Discharge Planning Intelligence Service - Groq Llama Edition
=============================================================
A FastAPI microservice for AI-powered discharge planning using Groq Llama-4 Maverick
for ultra-low latency medical discharge planning.

Powered by Groq Llama-4 Maverick (17B 128E Instruct) for fast discharge plan generation.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("discharge_planning_groq.log", encoding="utf-8"),
...
class Config:
    ...
    # Server configuration - different port to avoid conflict
    HOST: str = "0.0.0.0"
    PORT: int = int(os.getenv("PORT", "5053"))  # Render sets PORT automatically
...
class DischargePlanRequest(BaseModel):
    ...
    icd_code: str = Field(..., description="Primary ICD-10 code for the diagnosis", pattern=r"^[A-Z][0-9]{2}(\.[A-Z0-9]{1,4})?$")
...
# Response models aligned to the frontend DischargePlanning.tsx schema
...
class GroqService:
    """Service for Groq Llama API interactions with robust prompt + parsing"""
    ...
    def __init__(self):
        ...
        if is_placeholder:
            ...
            logger.info(f"Groq Service initialized with API key: {self.api_key[:10]}...")
            logger.info(f"Using Groq model: {self.model}")
            config.ENABLE_MOCK_MODE = False

    def _get_system_prompt(self) -> str:
        """System prompt for Groq to understand discharge planning context"""
        return (
    ...
    async def generate_discharge_plan(self, patient_id: str, icd_code: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        user_prompt = self._build_prompt(patient_id, icd_code, context)

        for attempt in range(self.max_retries):
            try:
                logger.info(f"🔍 Attempt {attempt + 1}/{self.max_retries} - Calling Groq API for discharge planning")
                ...
                if response.status_code == 200:
                    ...
                    return self._parse_groq_response(result, patient_id)
                else:
    ...
    def _parse_groq_response(self, groq_response: Dict[str, Any], patient_id: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(f"GROQ_RAW_RESPONSE: {content[:500]}...")  # Log first 500 chars
            ...
            logger.info("✅ Successfully parsed and validated Groq discharge plan response")
            return validated
    ...
    def _enforce_schema(self, parsed: Dict[str, Any], patient_id: str) -> Dict[str, Any]:
        ...
        if total_cost is None and devices:
    ...
    def _get_mock_response(self, patient_id: str, icd_code: str) -> Dict[str, Any]:
        ...
        mock = {
            "discharge_summary": {
                "primary_diagnosis": "Mock Diagnosis (Groq)",
                "secondary_diagnoses": ["E11.9"],
                "treatment_summary": "Mock treatment summary for testing Groq service.",
                "discharge_date": None,
...
class RequestLogger:
    ...
    def log_request(request_id: str, endpoint: str, data: Dict[str, Any]):
        ...
        with open("discharge_requests_groq.jsonl", "a", encoding="utf-8") as f:
    ...
    def log_response(request_id: str, endpoint: str, data: Dict[str, Any], status_code: int):
        ...
        with open("discharge_responses_groq.jsonl", "a", encoding="utf-8") as f:
            f.write(json.dumps(log_entry) + "\n")
    ...
    def log_error(request_id: str, endpoint: str, error: str):
        ...
        with open("discharge_errors_groq.jsonl", "a", encoding="utf-8") as f:
            f.write(json.dumps(log_entry) + "\n")
...
async def lifespan(app: FastAPI):
    logger.info(f"Starting Discharge Planning Service (Groq) on {config.HOST}:{config.PORT}")
    ...
    if config.ENABLE_MOCK_MODE:
        ...
        logger.info("✅ API key configured - Ready to use live Groq AI for discharge planning")
    
    yield
    logger.info("Shutting down Discharge Planning Service (Groq)")


app = FastAPI(
    title="Discharge Planning Intelligence Service - Groq Edition",
    description="AI-powered discharge planning using Groq Llama-4 Maverick for ultra-low latency",
    version="1.0.0",
    lifespan=lifespan
)
...
async def root():
    return {
        "service": "Discharge Planning Intelligence - Groq Edition",
        "status": "operational",
        "version": "1.0.0",
        "llm_provider": "Groq",
...
@app.post("/generate-discharge-plan-groq", response_model=DischargePlanResponse)
async def generate_discharge_plan(
    ...
    request_logger.log_request(request_id, "/generate-discharge-plan-groq", request.dict())
    ...
    try:
        ...
        llm_response = await groq_service.generate_discharge_plan(
            request.patient_id,
            request.icd_code,
            context
        )
        ...
        request_logger.log_response(request_id, "/generate-discharge-plan-groq", response_model.dict(), 200)
        ...
        request_logger.log_error(request_id, "/generate-discharge-plan-groq", error_msg)
